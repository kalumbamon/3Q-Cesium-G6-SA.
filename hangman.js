...
const words = ["aloud", "grape", "lemon", "mango", "peach", "shirt", "melon", "plums", "baked", "baths",
               "brims", "mouse", "canoe", "crabs", "crane", "honey", "crowd", "dares", "decoy", "downs", 
               "sigma", "dries", "dupes", "extol", "faces", "yeast", "yoked", "young", "wrong", "right",
               "years", "write", "whims", "vicar", "viper", "twist", "typed", "thief", "thong", "third",
               "sweat", "squad", "sibyl", "salon", "hands", "beads", "death", "table", "chair", "tiger"] ;
...
function checkLetter() {
    ...
    if (score === selectedWord.length || health === 0) {
        ...
        setTimeout(() => {
            alert(score === selectedWord.length ? "You already won!(Reload the page if you want to play again)" : "Game over!(Reload the page if you want to play again)");
        }, 300);
        return;
    }
    ...
    if (health === 0) {
        ...
        updateLivesDisplay();
        setTimeout(() => {
            alert("Game Over!(Reload the page if you want to play again)");
        }, 300);
    } else if (score === selectedWord.length) {
        updateHangman();
        updateLivesDisplay();
        setTimeout(() => {
            alert("You Win!(Reload the page if you want to play again)");
        }, 300);
    }
}